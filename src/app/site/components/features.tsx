...
const items = [
  {
    icon: () => (
      <svg className="w-5 h-5 text-brand-600 lg:w-6 lg:h-6 dark:text-brand-300" fill="currentColor" viewBox="0 0 20 20">
        <path
          fill-rule="evenodd"
        ...
        <div className="space-y-8 md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-12 md:space-y-0">
          {items.map((item, index) => (
            <div key={`${item.title}-${index}`}>
              <div className="flex justify-center items-center mb-4 w-10 h-10 rounded-full bg-brand-100 lg:h-12 lg:w-12 dark:bg-brand-900">
                {item.icon()}
              </div>
